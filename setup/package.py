...
# Utils
#----------------------------------------------------------
exec(open(join(dirname(__file__), '..', 'eagle', 'release.py'), 'rb').read())
...
def publish(o, type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/eagle_*.%s" % (o.build_dir, extension))
        if release:
            published.append(_publish(o, release[0]))
    return published

class EagleDocker(object):
    def __init__(self):
...
def _prepare_build_dir(o, win32=False):
    ...
    system(cmd + ['%s/' % o.eagle_dir, o.build_dir])
    for addon_path in glob(join(o.build_dir, 'addons/*')):
        if addon_path.split(os.path.sep)[-1] not in ADDONS_NOT_TO_PUBLISH:
            try:
                shutil.move(addon_path, join(o.build_dir, 'eagle/addons'))
            except shutil.Error as e:
                # Thrown when the add-on is already in eagle/addons (if _prepare_build_dir
...
def build_tgz(o):
    ...
    system(['mv', glob('%s/dist/eagle-*.tar.gz' % o.build_dir)[0], '%s/eagle_%s.%s.tar.gz' % (o.build_dir, version, timestamp)])
    system(['mv', glob('%s/dist/eagle-*.zip' % o.build_dir)[0], '%s/eagle_%s.%s.zip' % (o.build_dir, version, timestamp)])

def build_deb(o):
    # Append timestamp to version for the .dsc to refer the right .tar.gz
    cmd=['sed', '-i', '1s/^.*$/eagle (%s.%s) stable; urgency=low/'%(version,timestamp), 'debian/changelog']
    ...
    wildcards = ['eagle_{}'.format(wc) for wc in ('*.deb', '*.dsc', '*_amd64.changes', '*.tar.gz', '*.tar.xz')]
    move_glob(build_dir_parent, wildcards, o.build_dir)

def build_rpm(o):
    system(['python3', 'setup.py', '--quiet', 'bdist_rpm'], o.build_dir)
    system(['mv', glob('%s/dist/eagle-*.noarch.rpm' % o.build_dir)[0], '%s/eagle_%s.%s.noarch.rpm' % (o.build_dir, version, timestamp)])

def build_exe(o):
    KVMWinBuildExe(o, o.vm_winxp_image, o.vm_winxp_ssh_key, o.vm_winxp_login).start()
    system(['cp', glob('%s/openerp*.exe' % o.build_dir)[0], '%s/eagle_%s.%s.exe' % (o.build_dir, version, timestamp)])
...
def _prepare_testing(o):
    ...
    if not o.no_tarball:
        ...
        subprocess.call(["cp", "package.dfsrc", os.path.join(o.build_dir, "docker_src", "Dockerfile")],
                        cwd=os.path.join(o.eagle_dir, "setup"))
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_src")],
                        cwd=os.path.join(o.eagle_dir))
        subprocess.call(["docker", "build", "-t", "eagle-%s-src-nightly-tests" % docker_version, "."],
    ...
    if not o.no_debian:
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_debian")],
                        cwd=os.path.join(o.eagle_dir))
        subprocess.call(["docker", "build", "-t", "eagle-%s-debian-nightly-tests" % docker_version, "."],
    ...
    if not o.no_rpm:
        ...
        subprocess.call(["cp", "package.dffedora", os.path.join(o.build_dir, "docker_fedora", "Dockerfile")],
                        cwd=os.path.join(o.eagle_dir, "setup"))
        subprocess.call(["docker", "build", "-t", "eagle-%s-fedora-nightly-tests" % docker_version, "."],
                        cwd=os.path.join(o.build_dir, "docker_fedora"))

def test_tgz(o):
    logging.info('Testing tarball in docker')
    with docker('eagle-%s-src-nightly-tests' % docker_version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.tar.gz'
        wheezy.system("service postgresql start")
        wheezy.system('pip3 install /opt/release/%s' % wheezy.release)
        wheezy.system("useradd --system --no-create-home eagle")
        wheezy.system('su postgres -s /bin/bash -c "createuser -s eagle"')
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('mkdir /var/lib/eagle')
        wheezy.system('chown eagle:eagle /var/lib/eagle')
        wheezy.system('su eagle -s /bin/bash -c "eagle -d mycompany -i base --stop-after-init"')
        wheezy.system('su eagle -s /bin/bash -c "eagle -d mycompany &"')

def test_deb(o):
    logging.info('Testing deb package in docker')
    with docker('eagle-%s-debian-nightly-tests' % docker_version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.deb'
        wheezy.system("service postgresql start")
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('/usr/bin/dpkg -i /opt/release/%s' % wheezy.release)
        wheezy.system('/usr/bin/apt-get install -f -y')
        wheezy.system('su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany -i base --stop-after-init"')
        wheezy.system('su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany &"')

def test_rpm(o):
    logging.info('Testing rpm in docker')
    with docker('eagle-%s-fedora-nightly-tests' % docker_version, o.build_dir, o.pub) as fedora24:
        ...
        fedora24.system('su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany -i base --stop-after-init"')
        fedora24.system('su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany &"')
...
def options():
    ...
    # Windows VM
    op.add_option("", "--vm-winxp-image", default='/home/eagle/vm/win1036/win10_winpy36.qcow2', help="%default")
    op.add_option("", "--vm-winxp-ssh-key", default='/home/eagle/vm/win1036/id_rsa', help="%default")
    ...
    o.work_addons = join(o.work, 'eagle', 'addons')
