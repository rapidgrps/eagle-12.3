...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, cardnumber, expiration_date, card_code):
        ...
        # merchantCustomerId is EAGLE-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(profile, "merchantCustomerId").text = ('EAGLE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        # merchantCustomerId is EAGLE-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(customer, "merchantCustomerId").text = ('EAGLE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
