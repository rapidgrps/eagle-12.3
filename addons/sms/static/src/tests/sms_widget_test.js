...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', function (assert) {
        ...
        // GSM-7
        form.$('.o_input').val("Hello from Eagle").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 chars, fits in 1 SMS (GSM7) ', 'Should be "15 chars, fits in 1 SMS (GSM7) " for "Hello from Eagle"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from Eagle\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 chars, fits in 1 SMS (GSM7) ', 'Should be "17 chars, fits in 1 SMS (GSM7) " for "Hello from Eagle\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from Eagle").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 chars, fits in 1 SMS (UNICODE) ', 'Should be "15 chars, fits in 1 SMS (UNICODE) " for "Hêllo from Eagle"');
        // GSM-7 with 160c
